...
public class CustomMessageSender {
    ...
    public void sendMessage(ClientDTO clientDTO) {
       // final Client message = new Client("Hello there!", new Random().nextInt(50), false);

        log.info("Sending message...");
        rabbitTemplate.convertAndSend(MessagingApplication.EXCHANGE_NAME, MessagingApplication.ROUTING_KEY, clientDTO);
    }

    public void sendMessage(Client client) {
        // final Client message = new Client("Hello there!", new Random().nextInt(50), false);
        log.info("Sending message...");
        rabbitTemplate.convertAndSend(MessagingApplication.EXCHANGE_NAME, MessagingApplication.ROUTING_KEY, client);
    }
